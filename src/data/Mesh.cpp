...
struct _lua_vec
{
	...
	{
		vector<lua_Number> vec = mapper.get_numbers();
		assert(count == vec.count());
		for (render::uint8 i = 0; i < count; i++)
		{
			value[i] = vec[i];
		}
	}
...
void Mesh::from_lua(const gb::utils::luatable_mapper & mapper)
{
	...
	if (mapper.has_key(GB_RENDER_VTXVAR_POS))
		_mpVtxVars.insert(std::pair<const string, vtxVar>(GB_RENDER_VTXVAR_POS, mapper.get_tables_by_key<_lua_vec<3>>(GB_RENDER_VTXVAR_POS)));
	else
		logger::Instance().error("Mesh::from_lua no gb_vtxVar_pos key found@ " + mapper.GetFile());

	//idx
	if (mapper.has_key(GB_RENDER_VTXVAR_IDX))
		_mpVtxVars.insert(std::pair<string, vtxVar>(GB_RENDER_VTXVAR_IDX, mapper.get_integers_by_key(GB_RENDER_VTXVAR_IDX)));
	else
		logger::Instance().error("Mesh::from_lua no gb_vtxVar_idx key found@ " + mapper.GetFile());

	//uv(optional)
	if(mapper.has_key(GB_RENDER_VTXVAR_UV))
		_mpVtxVars.insert(std::pair<string, vtxVar>(GB_RENDER_VTXVAR_UV, mapper.get_tables_by_key<_lua_vec<2>>(GB_RENDER_VTXVAR_UV)));
}
